...
export class WeatherService {
  ...
  getCityWeatherByName(city: string, metric: 'metric' | 'imperial' = 'metric'): Subject<string> {
    ...
    this.http.get(
      `https://api.openweathermap.org/data/2.5/weather?q=${city}&appid=87a100579112bc2903ea22c94554eedd`)
      .subscribe((data) => {
  ...
  getCurrentTemp(city: string, metric: 'metric' | 'imperial' = 'metric'): Subject<number> {
    ...
    this.http.get(
      `https://api.openweathermap.org/data/2.5/weather?q=${city}&appid=87a100579112bc2903ea22c94554eedd&units=metric`)
  ...
  getWeatherName(city: string, metric: 'metric' | 'imperial' = 'metric'): Subject<String> {
    const dataSubject = new Subject<String>();
    this.http.get(
      `https://api.openweathermap.org/data/2.5/weather?q=${city}&appid=87a100579112bc2903ea22c94554eedd`)
      .subscribe((weather: any) => {
        dataSubject.next(weather.name);
      });
    return dataSubject;
  }
  ...
  getMaxTemp(city: string, metric: 'metric' | 'imperial' = 'metric'): Subject<number> {
    ...
    let max: number;
    this.http.get(
      `https://api.openweathermap.org/data/2.5/forecast?q=${city}&appid=87a100579112bc2903ea22c94554eedd&units=metric`)
      .subscribe((weather: any) => {
